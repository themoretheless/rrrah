...
use crate::decode_gate::DecodeGate;
...
impl RawPrefetcher {
    pub fn new(cache_root: Option<PathBuf>, no_cache: bool, decode_gate: Arc<DecodeGate>) -> Self {
                    ...
                    while let Ok(command) = commands.recv() {
                        for path in command.paths {
                            ...
                            match cache.store(key, &output.mosaic) {
                                Ok(_) => log::debug!("prefetched full RAW mosaic: {}", path.display()),
                                Err(error) => {
    ...
    pub fn finish_foreground_and_submit(&self, gallery: &[PathBuf], selected: usize) {
        ...
        if !self.enabled || selected >= gallery.len() {
        ...
        while self.pending.try_recv().is_ok() {}
        let _ = self.tx.try_send(RawPrefetchCommand { generation, paths });
